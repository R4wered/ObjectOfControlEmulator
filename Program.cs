...
using IKM724NetBasics.Constants;
...
namespace AutomatedLineOfDairyProducts
{
    class Program
    {
        ...
        private static LineOfDairyProducts lineOfDairyProducts = new LineOfDairyProducts();
        ...
        static async Task Main(string[] args)
        {
            ...
            var sensorFactories = new Dictionary<string, Func<Sensor>>
            {
                ...
                {"InnerHumidity",() => new HumiditySensor("Humidity", "No description") },
                {"InnerpH",() => new pHSensor("pH", "No description") },
                {"InnerFillingLevel",() => new FillingLevelSensor("Filling level", "No description") },
            };
            ...
            foreach (var indicator in deserializedResult)
            {
                if (sensorFactories.TryGetValue(indicator.Name, out var createSensor))
                {
                    ...
                    lineOfDairyProducts.Sensors.Add(sensor);
                }
                else
                {
        ...
        {
            lineOfDairyProducts.Monitor();

            var value = lineOfDairyProducts.Sensors.FirstOrDefault(x => x.Name == indicatorModel.Name);
